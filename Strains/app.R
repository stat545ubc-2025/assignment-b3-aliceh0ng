...
# ---- UI ----
ui <- fluidPage(
  # CSS to keep plate fixed 12×8 and not responsive-reflow
  # tags$head(
  #   tags$style(HTML("
  #   .plate-wrapper {
  #     overflow-x: auto;
  ...
  #   .plate-grid {
  #     display: grid;
  #     grid-template-columns: repeat(12, 60px); /* 12 columns fixed */
  #     grid-auto-rows: 60px;
  #     grid-gap: 4px;
  ...
  titlePanel("Plate Reader Time-course Viewer"),
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      fluidRow(
        ...
        column(
          width = 3,    # <- narrower table area
          h4("Well design table"),
          div(
            class = "design-table-wrapper",
            tableOutput("design_table")
          )
        )
      ),
...
server <- function(input, output, session) {
  ...
  # ---- Sheet selection UI (Time & OD600 mandatory) ----
  ...
  output$sheet_selector <- renderUI({
    ...
    validate(
      need(
        ...
        paste0(
          "Your file must contain sheets named: ",
    ...
    tagList(
      strong("Required sheets (always loaded):"),
      ...
      if (length(optional_present) > 0) {
        ...
        helpText("No optional sheets (GFP/BFP/RFP) found in this file.")
      }
    )
  })
  ...
  tidy_data <- reactive({
    ...
    all_meas <- purrr::map_dfr(meas_sheets, function(ch) {
      ...
      m2 %>%
        tidyr::pivot_longer(
          cols = setdiff(colnames(m2), c("Time", "Time_min", "Time_index")),
          names_to  = "Well",
          values_to = "Value"
        ) %>%
        dplyr::mutate(
  ...
  # Clear assignments
  observeEvent(input$clear_assignments, {
    plate_map(initial_plate)
  })

  # Show design table (any well with strain/condition)
  output$design_table <- renderTable({
    plate_map() %>%
      filter(!is.na(Strain) | !is.na(Condition),
             Strain != "" | Condition != "")
  })
  ...
  output$timeplot <- renderPlot({
    ...
    validate(
      need(nrow(design) > 0, "Assign at least one well a strain (Condition optional).")
    )
    ...
    validate(
      need(nrow(dat) > 0, "No finite data to plot for this strain / settings.")
    )
  ...
  output$strain_nav_ui <- renderUI({
    ...
    if (length(strains) == 0) {
      return(helpText("Assign at least one strain to wells to enable plotting."))
    }

    # keep index in range
    idx <- current_strain_idx()
    if (idx > length(strains)) {
      idx <- length(strains)
  ...
  observeEvent(input$prev_strain, {
    pm <- plate_map()
    strains <- pm %>%
      dplyr::filter(!is.na(Strain), Strain != "") %>%
      dplyr::pull(Strain) %>%
  ...
  observeEvent(input$next_strain, {
    pm <- plate_map()
    strains <- pm %>%
      dplyr::filter(!is.na(Strain), Strain != "") %>%
      dplyr::pull(Strain) %>%
  ...
  observeEvent(input$strain_select, {
    pm <- plate_map()
    strains <- pm %>%
      dplyr::filter(!is.na(Strain), Strain != "") %>%
      dplyr::pull(Strain) %>%
